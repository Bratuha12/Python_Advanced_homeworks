...
def find_url(url_short: str):
    """
    Функция ищет короткий URL в таблице url_shortener и возвращает
    соответствующий ему оригинальный URL или значение False при ненахождении
    объекта поиска.
    :param url_short: Искомый короткий URL.
    :return: Оригинальный URL или значение False.
    """
    try:
        with connection.cursor() as cur:
            query = f'SELECT * FROM url_shortener WHERE url_short ' \
                    f'LIKE "%/{url_short}"'
            cur.execute(query)
            record = cur.fetchone()
        return record[1]
...
def check_url(url_txt: str) -> bool:
    ...
    """
    if url_txt.startswith(('http://', 'https://', 'ftp://')):
        return True
    else:
        return False
...
def handler(request):
    ...
    if request.method == 'POST':
        ...
        if check_url(url_original):
            short_url = ''.join(
                ('http://', request.get_host(), '/', random_url()))
            insert_records(url_original, short_url)
        else:
